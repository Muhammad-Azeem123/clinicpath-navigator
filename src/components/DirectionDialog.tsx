...
export const DirectionDialog = ({ children, destinationName, destinationId, destinationLocation }: DirectionDialogProps) => {
  ...
  const handleGetDirections = async () => {
    ...
    setLoading(true);
    try {
      // If we have a specific destination ID, use it, otherwise try to find by name
      let targetLocationId = destinationId;
      
      if (!targetLocationId) {
        // Try to find location by name or room
        const foundLocation = locations.find(loc => 
          loc.name.toLowerCase().includes(destinationName.toLowerCase()) ||
          (loc.room && loc.room.toLowerCase().includes(destinationLocation?.toLowerCase() || ''))
        );
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4">
          ...
          <div className="p-4 bg-accent/20 rounded-lg">
            ...
            {destinationLocation && (
              <p className="text-xs text-muted-foreground">{destinationLocation}</p>
            )}
          </div>
